...
const SvgContainer:React.FC<Props> = ({
    ...
    const scalesOnUpdateEndHandler = () => {
        setScales(scales => {
            return {
                ...scales,
                // change last update time so the children know scales have changed
    ...
    const onZoomed = () => {
        ...
        const newRange = [ 0, width].map(d => d3Event.transform.apply(d) as number);
    ...
    const resizeHandler = () => {
        ...
        // const { svg } = svgContainerDataa;
        const { x, xScaleBand } = scales;

        //const newContainerWith = window.ineerWidth - 720;
